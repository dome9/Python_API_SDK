...
class OnBoardingCloudAccount(object):
	...
	def __init__(self, args):
		self.args = args
		self.d9client = Dome9ApiClient(apiKeyID=args.dome9ApiKeyID, apiSecret=args.dome9SecretKey)

	def onBoardingNewAccount(self):
		if self.args.cloudVendorType == 'aws':
			if not self.args.awsRoleArn or not self.args.awsRoleSecret:
			...
			cloudAccountObject = self.d9client.onBoardingAwsAccount(arn=self.args.awsRoleArn, secret=self.args.awsRoleSecret, name=self.args.dome9AccountName, allowReadOnly=self.args.awsAllowReadOnly, fullProtection= self.args.awsFullProtection)
			...
			cloudAccountObject = self.d9client.onBoardingAzureAccount(subscriptionID=self.args.azureSubscriptionID, name=self.args.dome9AccountName, tenantID=self.args.azureActiveDirectoryID, clientID=self.args.azureApplicationID, clientPassword=self.args.azureSecretKey, operationMode=self.args.azureOperationMode)
...
if __name__ == '__main__':
	...
	useExample = '''
	AWS:
	--dome9ApiKeyID sdfsdfssdf --dome9SecretKey sdfsdfssdf --cloudVendorType aws --awsRoleArn arn:aws:iam::111111111:role/Dome9-Connect --awsRoleSecret sdfsdfsdff --dome9OuID e21b3e8b-e02f-46df-bd70-8ce65ca8a3a5 --dome9AccountName production --dome9AdminRoleID 118187 --dome9ViewRoleID 118203
	
	Azure:
	--dome9ApiKeyID ddsfsdfsdf --dome9SecretKey sdfsdfssdf --cloudVendorType azure --azureSubscriptionID sdfsdfsdfsdfsd --azureActiveDirectoryID sdfsdsdfsdsdfsd --azureApplicationID sfsdfsdfsfdsdf --azureSecretKey sdfsfsfsfd --dome9OuID 92f9a334-bf29-48a5-9cf8-66a10efe51e6 --dome9AccountName production --dome9AdminRoleID 118881 --dome9ViewRoleID 118901 --azureOperationMode Manage
	'''
	...
	parser.add_argument('--dome9SecretKey',         required=True,  type=str, help='Dome9 secret key')
	parser.add_argument('--cloudVendorType',        required=True,  type=str, choices=OnBoardingCloudAccount.VENDOR_TYPES, help='type of cloud account vendor: aws, azure')
	parser.add_argument('--awsRoleArn',             required=False, type=str, help='The ARN of the Dome9-Connect role in your AWS account')
	parser.add_argument('--awsRoleSecret',          required=False, type=str, help='The external ID value used to create the role in your AWS account')
	parser.add_argument('--awsAllowReadOnly',       required=False, type=str, default=True,   choices=OnBoardingCloudAccount.AWS_ALLOW_READONLY, help='Default=True, set to True for Read-Only, and False for Full Protection')
	parser.add_argument('--awsFullProtection',      required=False, type=str, default=False,  choices=OnBoardingCloudAccount.AWS_FULL_PROTECTION, help='Default=False, set to True for to set the Security Groups in the account to Full-Protection in the course of onboarding, or False to leave them unchanged')
	...
	parser.add_argument('--dome9AccountName',       required=False, type=str, default='account-{}'.format(str(uuid.uuid4())[:8]), help='Default=account-randomString , accountName display on Dome9 console')
